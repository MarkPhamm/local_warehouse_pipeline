# 🔮 CFPB Complaint Response Predictor (ML Component)
...
The solution includes an end-to-end workflow: **Exploratory Data Analysis (EDA) → Feature Engineering → Model Training → Inference Engine → Streamlit Web App.**
...
├── src/
│   └── models/                    # Saved artifacts (xgboost.pkl, preprocessor.pkl, encoders)
├── database/                      # Connection to DuckDB (output from ELT pipeline)
├── docs/
│   ├── README_ML.md
````
